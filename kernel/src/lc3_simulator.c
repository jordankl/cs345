...
extern int charReadySemId;			// character has been entered
...
// initLC3Memory needs to run in kernel mode since it initializes as part
// of the core operating system before it is ready to start running
// various tasks.
...
int loadLC3Program(char* argv[])
{
	...
	switch (INTEGER(argv[0]))
	{
		...
		case 1:
		{
			...
			if ((error != E_END_OF_FILE) && error) fmsError(error);
		...
		default:
		{
			printf("Memory NOT loaded!  Illegal file type=%d\n", INTEGER(argv[1]));
			return -1;
		}
	}
	printf("Memory loaded!  PC=0x%04x\n", (unsigned int)startPC);
	return startPC;
} // end loadLC3Program
